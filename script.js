document.addEventListener("DOMContentLoaded", async function () {
  ...
  const dateFilterAll = document.getElementById("date-filter-all");
  const showLineupsBtn = document.getElementById("show-lineups-btn");
  ...
  function formatTeamName(name) {
    const team = teams.find(t => t.fullName === name);
    if (team) {
      const city = name.replace(team.briefName, "").trim();
      return `${city}<br>${team.briefName}`;
    }
    return name;
  }
  ...
  async function fetchPlayerStats(playerId, startDate, endDate) {
    ...
    const data = await response.json();
    const stats =
      data.stats[0]?.splits.find((split) => split.player.id === playerId)?.stat || {};
    return {
      avg: stats.avg || "-",
      obp: stats.obp || "-",
      ops: stats.ops || "-",
      ...
      battingOrder.forEach((player, index) => {
        ...
        playerRow.innerHTML = `
          ...
          <div class="player-stats">
            <span class="bat-pa">${player.pa || "-"}</span>
  ...
  async function renderGames(games) {
    ...
    for (const game of games) {
      ...
      gameHeaderDiv.innerHTML = `
        <span>${new Date(game.gameDate).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
          hour12: true,
        })}</span>
      ...
      const awayPitcherHand = awayTeam.probablePitcher?.pitchHand
        ? `${awayTeam.probablePitcher.pitchHand}HP`
        : "";
      const homePitcherHand = homeTeam.probablePitcher?.pitchHand
        ? `${homeTeam.probablePitcher.pitchHand}HP`
        : "";
      ...
      const formatTeamName = (name) => {
        const team = teamsData.find(t => t.fullName === name);
        if (team) {
          const city = name.replace(team.briefName, "").trim();
          return `${city}<br>${team.briefName}`;
        }
      ...
      matchupDiv.innerHTML = `
        <div class="team away-team" data-team-id="${awayTeam.team.id}">
          ...
          <div class="team-info">
            <span class="team-name">${formatTeamName(awayTeam.team.name)}</span>
            <span class="team-record">(${awayTeam.leagueRecord.wins}-${
        awayTeam.leagueRecord.losses
      })</span>
            <span class="probable-pitcher">${
              awayTeam.probablePitcher?.fullName || "TBD"
            }</span>
        ...
        <div class="team home-team" data-team-id="${homeTeam.team.id}">
          ...
          <div class="team-info">
            <span class="team-name">${formatTeamName(homeTeam.team.name)}</span>
            <span class="team-record">(${homeTeam.leagueRecord.wins}-${
      ...
      splitsDiv.innerHTML = `
        <div class="lineup-status" id="lineup-status-${game.gamePk}">
          <div class="away-lineup-status"></div>
          <div class="home-lineup-status"></div>
        </div>
      ...
      function setActiveClass(teamDiv) {
        ...
        const otherTeamTypeClass =
          otherTeamDiv === awayTeamDiv ? "awaytm-active" : "hometm-active";
        ...
        if (teamDiv.classList.contains("tm-active")) {
          ...
          teamLogo.src = getLogoUrl(
            teamId,
            teamDiv.querySelector(".team-name").textContent
          );
        } else {
      ...
      dropdown.addEventListener("click", function () {
        ...
        if (dropdown.classList.contains("tm-active")) {
          ...
          if (activeTeam) {
            ...
            teamLogo.src = getLogoUrl(
              teamId,
              activeTeam.querySelector(".team-name").textContent
            );
          }
          ranksDiv.classList.remove("tm-active");
          splitsDiv.classList.remove("awaytm-active", "hometm-active");
          lineupsDiv.classList.remove("awaytm-active", "hometm-active");
        } else {
          dropdown.classList.add("tm-active");
          arrow.innerHTML = "&#8593;";
          setActiveClass(homeTeamDiv);
  ...
  showLineupsBtn.addEventListener("click", function () {
    const homeTeams = document.querySelectorAll(".home-team");

    if (showLineupsBtn.textContent.includes("Show Lineups")) {
      homeTeams.forEach(homeTeam => {
        homeTeam.click();
      });
      showLineupsBtn.innerHTML = `Hide Lineups <i class="fa-solid fa-eye-slash"></i>`;
    } else {
      const dropdowns = document.querySelectorAll(".dropdown.tm-active");
      dropdowns.forEach(dropdown => {
        dropdown.click();
      });
      showLineupsBtn.innerHTML = `Show Lineups <i class="fa-solid fa-eye"></i>`;
    }
  });
});
